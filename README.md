### Advent of Code in eBPF (Days 1-3)

To gain experience in eBPF, I decided to try to code up [AoC 2025](https://adventofcode.com/) in eBPF.

As I had already done these problems in Python, the focus was on replicating the algorithms I happened to use given the constraints of eBPF. I also wanted to explore the variety of hook points available and gain some eBPF and Linux kernel intuition. From that perspective, mission accomplished.
...
The main files worth inspecting in each directory will be `aocdXp{1|2}.c` and `main.go`. The first is the actual eBPF program, and the second is the Golang helper, responsible for loading and attaching the eBPF program as well as populating/polling eBPF maps. In the case of Day3, both programs have the same entry-point: you just need to rrun the relevant make command with `PROBLEM=1` or `PROBLEM=2`.
...
* I used vmlinux.h (this is a file you generate) rather than kernel header files as I did before. This is the modern canonical way to do things, and it's great.
* I look forward to using eBPF for a real purpose.
* The "doing concurrency safely while allowing multiple inputs to be worked simultaneously" part was by far the most interesting and difficult part. That said, I think the overall lesson I've absorbed re: concurrency and eBPF is, more or less, "don't."
